...
static _z_task_slot_t *_z_task_slot_alloc(void) {
    for (int i = 0; i < Z_ZEPHYR_TASK_POOL_SIZE; i++) {
        ...
        if ((atomic_get(&slot->state) == _Z_TASK_SLOT_DETACHED) && (k_thread_join(&slot->thread, K_NO_WAIT) == 0) &&
...
z_result_t _z_task_init(_z_task_t *task, z_task_attr_t *attr, void *(*fun)(void *), void *arg) {
    ...
    task->_slot = slot;
    k_tid_t tid =
        k_thread_create(&slot->thread, stack, stack_size, _z_task_entry, slot, NULL, NULL, priority, 0, K_NO_WAIT);
