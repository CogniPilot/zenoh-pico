...
// Zephyr address-info type from <zephyr/net/socket.h>, forward-declared so
// this header does not pull the network stack into every include site. The
// zsock spelling is the real type name and exists with or without the POSIX
// layer (the unprefixed struct addrinfo is only a POSIX netdb.h alias for it).
